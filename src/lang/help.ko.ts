...
export const helpKo = {
        ...
        "claudeCachingExperimental": "기존/classic Claude 프롬프트 캐싱은 모델 비용을 줄일 수 있지만, 리롤 없이 사용하면 비용이 증가할 수도 있습니다. 실험적 기능이며 향후 동작이 변경될 수 있습니다.",
        ...
        "automaticCachePoint": "채팅이 끝난 후 캐싱 포인트가 없으면 자동으로 생성합니다. 지원되는 기존 Claude 및 ModelPreset Gemini 컨텍스트 캐시 경로에서 사용됩니다.",
        ...
        "claude1HourCaching": "기존/classic Claude 요청에서 프롬프트 캐시 TTL을 기본 5분 대신 1시간으로 설정합니다. 같은 컨텍스트로 자주 보낼 때 비용 절감 폭이 커지지만 1시간 캐시는 과금 단가가 다릅니다. 프리셋 요청에는 적용되지 않습니다.",
        "claudeBatching": "기존/classic Claude 요청에만 Claude 배치(Batch) API를 사용합니다. ModelPreset Anthropic 배치는 프리셋의 service_tier=batch 값으로 설정합니다. 비용이 더 저렴하지만 응답이 몇 분~수 시간 지연될 수 있습니다. 프리셋 요청에는 적용되지 않습니다.",
        ...
        "unrecommendedClaudeCachingRetrival": "기존/classic Claude 응답을 캐싱해 같은 요청 시 재사용을 시도합니다. 캐시 무효화 조건이 까다로워 의도하지 않은 결과를 반환할 수 있어 비권장입니다. 프리셋 요청에는 적용되지 않습니다.",
