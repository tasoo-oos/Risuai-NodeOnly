...
export const helpCn = {
        ...
        "claude1HourCaching": "在既有/classic Claude 请求中，将提示缓存 TTL 从默认 5 分钟改为 1 小时。相同上下文频繁发送时可进一步节省成本，但 1 小时缓存的计费价格不同。不会应用于预设请求。",
        "claudeBatching": "仅对既有/classic Claude 请求使用 Claude Batch API。ModelPreset Anthropic 批处理请改用预设中的 service_tier=batch 值。成本更低，但响应可能延迟数分钟到数小时。不会应用于预设请求。",
        "claudeCachingExperimental": "Claude 缓存是实验性功能，可减少模型成本。但若在不使用重滚（reroll）回应的情况下激活，则可能增加成本。实验性功能可能不稳定，且未来可能会有所变动。",
        "unrecommendedClaudeCachingRetrival": "尝试为重复请求复用已缓存的既有/classic Claude 响应。缓存失效条件较复杂，可能返回非预期结果，因此不建议使用。不会应用于预设请求。",
