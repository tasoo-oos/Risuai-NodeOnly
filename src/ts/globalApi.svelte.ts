...
/**
 * Records a completed fetch in the server request log. Kept for the
 * Anthropic batch-job path (process/request/anthropicBatchJob.ts), which
 * passes this as its logFetch callback; the in-memory fetchLog array it once
 * wrote to was replaced by the server-side requestLog collection.
 *
 ...
 * @param {number} [arg.status] - The HTTP status of the response.
 * @returns {number} - Always 0 (legacy return; the entry lives server-side).
 */
export function addFetchLog(arg: {
    ...
    recordRequestLog({
        ...
        requestHeaders: stringify(arg.headers ?? {}),
        requestBody: stringify(arg.body),
        responseBody: stringify(arg.response),
